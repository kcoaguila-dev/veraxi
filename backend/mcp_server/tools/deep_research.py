...
def mcp_deep_research(
    ...
    logger.info(
        f"Deep Research triggered for query: '{query}' on original tenant: {tenant_id}"
    )
    ...
    if not urls:
        ...
        raw_results = provider.search(
            query, language="en", max_results=max_results, web_settings=web_settings
        )
    ...
    if not urls:
        return {
            "error": "No valid URLs found to ingest.",
            "query": query,
            "results": [],
        }

    logger.info(
        f"Deep Research found {len(urls)} URLs. Ingesting concurrently into {ephemeral_tenant_id}..."
    )
    ...
    try:
        ...
        logger.info(
            f"Deep Research ingestion complete in {ephemeral_tenant_id}. Running merge_rank..."
        )
        ...
        return {
            ...
            "urls_ingested": urls,
            "ingestion_statuses": [
                res.get("status") or res.get("error") for res in ingestion_results
            ],
        ...
        logger.info(
            f"Deep Research complete. Wiping ephemeral tenant: {ephemeral_tenant_id}"
        )
        ...
        try:
            ...
            qdrant_client.delete_tenant(
                config.qdrant_collection_name, ephemeral_tenant_id
            )
