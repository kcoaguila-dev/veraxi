        ...
        audio_clip = otio.schema.Clip(
            ...
            media_reference=otio.schema.ExternalReference(
                ...
                available_range=otio.opentime.TimeRange(
                    start_time=otio.opentime.from_seconds(0.0), duration=duration
                ),
            ),
...
def build_fcpxml(clips: list[dict[str, Any]]) -> str:
    ...
    for i, clip in enumerate(clips):
        ...
        xml.append(
            f'            <clip name="{clip.get("character", "Voice")}" duration="{dur_str}" offset="{start_str}">'
        )
        ...
        xml.append(
            f'              <title name="{clip.get("character", "Title")}" offset="0s" duration="{dur_str}">'
        )
