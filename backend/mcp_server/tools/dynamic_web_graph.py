...
DEFAULT_SCHEMA = {
    ...
    "relations": {
        ...
        "Location": {"Location": ["LOCATED_IN", "BORDERS"]},
        "Event": {
            "Location": ["OCCURRED_IN"],
            "Person": ["INVOLVED", "ATTENDED"],
            "Organization": ["HOSTED", "SPONSORED"],
        },
        "Concept": {"Concept": ["RELATED_TO", "CAUSES", "DEPENDS_ON"]},
        "Product": {"Organization": ["CREATED_BY"], "Concept": ["USES_TECHNOLOGY"]},
    },
}
...
def _extract_graph_from_result(result: dict) -> dict:
    ...
    entities, relations = extract_entities_and_relations_fast(
        text=text, schema=DEFAULT_SCHEMA, language="en"
    )
...
def mcp_dynamic_web_graph(
    ...
    return {
        "graph": {"entities": aggregated_entities, "relations": aggregated_relations},
        "sources": sources,
    }
