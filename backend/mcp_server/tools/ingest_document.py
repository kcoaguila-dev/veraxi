    ...
    try:
        if file_path:
            ...
            json_data = {
                ...
                "language": language,
                "custom_stop_words": [w.strip() for w in custom_stop_words.split(",")]
            ...
            with httpx.Client(timeout=300) as client:
                response = client.post(
                    f"{api_base}/admin/ingest/url", json=json_data, headers=headers
                )
        ...
        if wait_for_completion and "job_id" in result:
            ...
            while True:
                ...
                if status_res.get("status") not in [
                    "queued",
                    "in_progress",
                    "deferred",
                ]:
...
def mcp_get_ingest_status(tenant_id: str, job_id: str) -> dict[str, Any]:
    ...
    try:
        with httpx.Client(timeout=10) as client:
            response = client.get(
                f"{api_base}/admin/ingest/status/{job_id}", headers=headers
            )
            response.raise_for_status()
