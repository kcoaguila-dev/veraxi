...
@mcp_server.tool(name='mcp_search_vectors', description='Semantic search over documents')
...
@mcp_server.tool(name='mcp_insert_graph_nodes', description='Insert structured nodes and relations into the Neo4j Knowledge Graph. The Host AI should extract these from unstructured text first.')
...
@mcp_server.tool(name='mcp_insert_vectors', description='Generate embeddings and insert text chunks into the Qdrant Vector Database.')
...
@mcp_server.tool(name='mcp_merge_rank', description='Perform a unified GraphRAG search. It searches vectors using query_text and traverses the graph from entity_name, then fuses the results using Reciprocal Rank Fusion.')
def mcp_merge_rank(query_text: str, entity_name: str, limit: int = 10, max_hops: int = 2) -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
...
@mcp_server.tool(name='mcp_get_graph_schema', description='Retrieves all unique Node Labels and Relationship Types currently in the Neo4j database. Call this before inserting data to understand the current schema.')
...
@mcp_server.tool(name='mcp_delete_entity', description='Deletes a specific entity and all its relationships from Neo4j.')
...
@mcp_server.tool(name='mcp_delete_document', description='Deletes a specific document chunk from Qdrant using its document ID.')
...
@mcp_server.tool(name='mcp_update_entity', description='Updates the properties of an existing Neo4j entity. Only provide the properties you want to add or overwrite.')
...
@mcp_server.tool(name='mcp_get_database_stats', description='Retrieves high-level statistics about the size of the database (nodes, relationships, vectors).')
...
@mcp_server.tool(name='mcp_run_community_detection', description='Runs a Graph Data Science community detection algorithm to find clusters of connected entities.')
def mcp_run_community_detection(min_size: int = 2) -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
    from backend.mcp_server.handlers.tool_registry import _handle_run_community_detection
    res = _handle_run_community_detection(args, tenant_id)
    # The old handler returns list[TextContent]. FastMCP expects str.
    return res[0].text

@mcp_server.tool(name='mcp_delete_relationship', description='Deletes a specific relationship edge between two entities without deleting the entities themselves.')
...
@mcp_server.tool(name='mcp_update_document_metadata', description='Updates or adds metadata properties to an existing vector document chunk in Qdrant.')
def mcp_update_document_metadata(document_id: str, payload: dict) -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
    from backend.mcp_server.handlers.tool_registry import _handle_update_document_metadata
    res = _handle_update_document_metadata(args, tenant_id)
    # The old handler returns list[TextContent]. FastMCP expects str.
    return res[0].text

@mcp_server.tool(name='mcp_evaluate_grounding', description='Evaluates what percentage of a generated response is mathematically supported by the retrieved context. Returns a float between 0.0 and 1.0.')
...
@mcp_server.tool(name='mcp_web_search', description='Fallback mechanism to search the live web when internal retrieval yields insufficient context. Returns a list of JSON snippets.')
...
@mcp_server.tool(name='mcp_skills', description='Lists available agentic skills for tool augmentation.')
def mcp_skills() -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
...
@mcp_server.tool(name='mcp_run_code', description='Executes Python code in a secure sandbox and returns the stdout/stderr.')
...
@mcp_server.tool(name='mcp_list_artifacts', description='Lists all artifacts stored in the workspace for the current tenant.')
...
@mcp_server.tool(name='mcp_read_artifact', description='Reads the content of a specific artifact by its name.')
...
@mcp_server.tool(name='mcp_get_ingest_status', description='Polls the status of an active ingestion job.')
...
@mcp_server.tool(name='mcp_dynamic_web_graph', description='Dynamic Web Search GraphRAG - searches the web and returns a structured Knowledge Graph of entities and relations instead of raw text snippets, allowing for high-accuracy multi-hop reasoning over live data.')
def mcp_dynamic_web_graph(query: str, language: str = "en", max_results: int = 5) -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
...
@mcp_server.tool(name='mcp_export_data', description='Serializes and exports dynamically structured datasets into agnostic, interoperable file formats (CSV, OTIO, FCPXML). Facilitates seamless data handoff to external analytical pipelines, data science environments, and professional non-linear editing (NLE) systems without imposing domain-specific schemas.')
def mcp_export_data(data: list, format: str = "csv", output_name: str = "veraxi_export") -> str:
    tenant_id = tenant_context.get()
    args = {k: v for k, v in locals().items() if k != 'tenant_id'}
