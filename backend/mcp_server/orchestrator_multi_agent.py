...
class SkepticEvaluation(BaseModel):
    critique: str = Field(description="Detailed explanation of any factual errors, missing citations, or hallucinations. Leave empty if perfect.")
    is_approved: bool = Field(description="True if the draft is perfectly supported by the research data, False otherwise.")
...
async def skeptic_node(state: DebateState):
    ...
    config = get_config()
    llm = _create_chat_llm(config.llm_model_name, config.llm_api_key)
    structured_llm = llm.with_structured_output(SkepticEvaluation, method="function_calling")
...
def get_debate_workflow():
    ...
    workflow.add_conditional_edges("skeptic", debate_router, {"writer": "writer", END: END})
    
    return workflow


async def mcp_agentic_debate(query: str, tenant_id: str, tool_settings: dict | None = None) -> dict:
