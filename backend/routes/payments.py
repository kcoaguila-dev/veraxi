...
def _activate_tenant_subscription(tenant_id: str | None, config, redis):
    ...
    try:
        ...
        supabase_client.table("users").update({"is_subscribed": True}).eq(
            "id", tenant_id
        ).execute()
...
def register_payment_routes(
    ...
    async def create_checkout_session(
        request: CheckoutSessionRequest, tenant_id: str = Depends(get_tenant_id)
    ):
        if not config.stripe_api_key:
            raise HTTPException(
                status_code=500, detail="Stripe is not configured on this server."
            )
    ...
    async def stripe_webhook(request: Request):
        ...
        if event["type"] == "checkout.session.completed":
            ...
            logger.info(
                f"💰 STRIPE PAYMENT RECEIVED for tenant: {tenant_id}! Activate their subscription."
            )
