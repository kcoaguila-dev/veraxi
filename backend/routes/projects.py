...
def register_project_routes(
    ...
    async def list_projects(request: Request, tenant_id: str = Depends(get_tenant_id)):
        projects_dict = await request.app.state.redis.hgetall(
            f"tenant:{tenant_id}:projects"
        )
        return {
            "projects": [
                {"id": pid.decode("utf-8"), "name": pname.decode("utf-8")}
    ...
    async def create_project(
        ...
        await request.app.state.redis.hset(
            f"tenant:{tenant_id}:projects", project_id, payload.name
        )
    ...
    async def rename_project(
        ...
        if not await request.app.state.redis.hexists(
            f"tenant:{tenant_id}:projects", project_id
        ):
    ...
    async def delete_project(
        project_id: str, request: Request, tenant_id: str = Depends(get_tenant_id)
    ):
        if not await request.app.state.redis.hexists(
            f"tenant:{tenant_id}:projects", project_id
        ):
        ...
        thread_projects = await request.app.state.redis.hgetall(
        ...
        for tid, pid in thread_projects.items():
            if pid.decode("utf-8") == project_id:
                await request.app.state.redis.hdel(
                    f"tenant:{tenant_id}:thread_projects", tid.decode("utf-8")
                )
        return {"status": "ok"}
    ...
    async def assign_project(
        ...
        if not payload.project_id:
            ...
            if not await request.app.state.redis.hexists(
                f"tenant:{tenant_id}:projects", payload.project_id
            ):
                raise HTTPException(status_code=404, detail="Project not found")
            await request.app.state.redis.hset(
                f"tenant:{tenant_id}:thread_projects", thread_id, payload.project_id
            )
        return {"status": "ok"}
