    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Generate a schema for the following text:\n\n{snippet}",
                },
            ],
...
def register_ingestion_routes(
    ...
    async def ingest_data(
        ...
        try:
            ...
            await _ensure_schema_exists(
                request.app.state.redis, tenant_id, ingest_request.text
            )
    ...
    async def ingest_memory(
        request: Request,
        memory_request: MemoryIngestRequest,
        tenant_id: str = Depends(verify_infrastructure_access),
    ):
        try:
            check_tenant_hard_cap(tenant_id, get_config())
            ...
            await _ensure_schema_exists(
                request.app.state.redis, tenant_id, content_with_tag
            )
    ...
    async def ingest_upload(
        ...
        try:
            ...
            if mime_type not in _ALLOWED_MIMES:
                os.unlink(tmp_path)
                logger.warning(
                    f"File upload blocked. Invalid magic bytes detected: {mime_type} for file {file.filename}"
                )
                raise HTTPException(
                    status_code=400,
            ...
            os.unlink(tmp_path)
            logger.info(
                f"Enqueueing {len(markdown_text)} bytes of markdown from {file.filename}"
            )
            parsed_stop_words = (
                [w.strip() for w in custom_stop_words.split(",")]
                if custom_stop_words
                else []
            )
            await _ensure_schema_exists(
                request.app.state.redis, tenant_id, markdown_text
            )
            job = await request.app.state.redis.enqueue_job(
                "process_ingestion_task",
                markdown_text,
    ...
    async def ingest_url(
        ...
        try:
            ...
            markdown_text = result.document.export_to_markdown()
            logger.info(
                f"Enqueueing {len(markdown_text)} bytes of markdown from {url_request.url}"
            )
            await _ensure_schema_exists(
                request.app.state.redis, tenant_id, markdown_text
            )
        ...
        if status == JobStatus.complete:
            ...
            if info and info.success:
                ...
                if (
                    ...
                    and result_data.get("status") == "error"
                ):
                    return {
                        "status": "error",
    ...
    async def get_schema(
        request: Request, tenant_id: str = Depends(verify_infrastructure_access)
    ):
        schema = await request.app.state.redis.get(f"tenant:{tenant_id}:schema")
        if schema:
            return json.loads(schema)
        raise HTTPException(
            status_code=404, detail="No schema defined for this tenant."
        )
    ...
    async def set_schema(
        ...
        await request.app.state.redis.set(
            f"tenant:{tenant_id}:schema", schema.model_dump_json()
        )
