...
def run_benchmark():
    ...
    print(
        f"Average Vector RAG Correctness (F1 Overlap): {avg_vector * 100:.1f}%  (n={len(scored_vector)})"
    )
    print(
        f"Average Hybrid GraphRAG Correctness (F1 Overlap): {avg_hybrid * 100:.1f}%  (n={len(scored_hybrid)})"
    )
    print("=" * 60)
