...
CORPUS_PATH = os.path.join(
    os.path.dirname(__file__), "..", "tests", "data", "graphrag_test_corpus.txt"
)
...
def generate_multihop(num_samples: int):
    ...
    dataset = load_dataset(
        "yixuantt/MultiHopRAG", "MultiHopRAG", split="train", streaming=True
    )
...
def generate_graphrag_bench(num_samples: int):
    ...
    dataset = load_dataset(
        "GraphRAG-Bench/GraphRAG-Bench", "medical", split="train", streaming=True
    )
    ...
    if num_samples > 0:
        ...
        for q_type, group in strata.items():
            ...
            stratum_sample_size = max(1, round(proportion * num_samples))
...
def generate():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        choices=["multihop", "graphrag_bench"],
        default="multihop",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of samples to extract. Set to 0 to extract the entire dataset.",
    )
    ...
    print(
        f"Generated test corpus at {CORPUS_PATH} with {len(corpus_paragraphs)} unique evidence paragraphs."
    )
    ...
    print(
        f"Generated golden dataset at {DATASET_PATH} with {len(evaluation_dataset)} {args.dataset} benchmark questions."
    )
