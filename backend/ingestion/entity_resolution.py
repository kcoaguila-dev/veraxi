...
def _find_matching_cluster(name: str, clusters: List[List[Dict[str, Any]]]) -> List[Dict[str, Any]] | None:
    for cluster in clusters:
        canonical_name = cluster[0].get("name", "")
        similarity = difflib.SequenceMatcher(None, name.lower(), canonical_name.lower()).ratio()
        if similarity >= 0.85:
            return cluster
    return None
