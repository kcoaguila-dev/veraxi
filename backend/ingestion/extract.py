...
def _is_valid_entity(ent_type: Any, name: Any, allowed_entities: list[str]) -> bool:
    return ent_type in allowed_entities and isinstance(name, str) and bool(name.strip())
...
def _validate_relations(
    ...
    allowed_relations: dict,
) -> list[dict[str, str]]:
    valid_relations = []
    for rel in relations:
        valid_rel = _validate_single_relation(
            rel, entity_name_to_type, allowed_relations
        )
        if valid_rel:
...
def validate_extraction(
    ...
    valid_entities, entity_name_to_type = _validate_entities(
        entities, schema["entities"]
    )
    valid_relations = _validate_relations(
        relations, entity_name_to_type, schema.get("relations", {})
    )
...
def _extract_relations_from_sentence(sent, entity_name_to_type, allowed_relations):
    ...
    relations = []
    ents_in_sent = [
        ent.text.strip() for ent in sent.ents if ent.text.strip() in entity_name_to_type
    ]
    ...
    if len(ents_in_sent) >= 2:
        for i in range(len(ents_in_sent)):
            for j in range(i + 1, len(ents_in_sent)):
                ...
                if allowed_t1_t2:
                    relations.append(
                        {"from_entity": e1, "to_entity": e2, "type": allowed_t1_t2[0]}
                    )

                allowed_t2_t1 = allowed_relations.get(t2, {}).get(t1, [])
                if allowed_t2_t1:
                    relations.append(
                        {"from_entity": e2, "to_entity": e1, "type": allowed_t2_t1[0]}
                    )
...
def extract_entities_and_relations_fast(
    ...
    for ent in doc.ents:
        ...
        entities.append({"type": mapped_type, "name": name, "properties": {}})
    ...
    for sent in doc.sents:
        relations.extend(
            _extract_relations_from_sentence(
                sent, entity_name_to_type, allowed_relations
            )
        )
