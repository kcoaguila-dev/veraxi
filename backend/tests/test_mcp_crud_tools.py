...
def test_delete_vector(qdrant_client, patch_env, test_tenant):
    ...
    records, _ = qdrant_client.client.scroll(
        collection_name=config.qdrant_collection_name, scroll_filter=None, limit=10
    )
...
def test_run_analytics(neo4j_client, test_tenant):
    ...
    with patch(
        "backend.mcp_server.tools.run_analytics.get_community_detection"
    ) as mock_gcd:
...
def test_update_document_metadata(qdrant_client, patch_env, test_tenant):
    ...
    result = update_document_metadata(
        doc_id, {"new_key": "new_value", "original": False}, tenant_id=test_tenant
    )
    assert "Successfully updated metadata" in result

    # Verify
    records = qdrant_client.client.retrieve(
        collection_name=config.qdrant_collection_name, ids=[doc_id]
    )
...
def test_update_entity(neo4j_client, test_tenant):
    ...
    result = update_entity(
        "Update Me", {"age": 31, "city": "New York"}, tenant_id=test_tenant
    )
