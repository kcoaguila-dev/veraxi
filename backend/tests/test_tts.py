...
async def test_fish_audio_adds_model_header_for_free_tier():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        assert headers.get("model") == "s2.1-pro-free", "The 'model' header must be set to the free tier model string"
        assert headers.get("Authorization") == "Bearer fake_api_key"
        
        # Ensure 'model' is NOT in the JSON payload
        payload = kwargs.get("json", {})
        assert "model" not in payload, "The 'model' parameter must be passed in the headers, not the JSON payload"
