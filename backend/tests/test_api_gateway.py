...
def mock_redis():
    ...
    with patch(
        "backend.api_gateway.create_pool", new_callable=AsyncMock
    ) as mock_create_pool:
...
@patch("backend.routes.chat.moderate_text")
@patch("backend.routes.chat.answer_question")
@patch("backend.routes.chat.generate_chat_title")
def test_chat_endpoint_non_streaming(
    mock_gen_title, mock_answer, mock_moderate, client
):
...
@patch("backend.routes.admin.Neo4jStorageClient.from_config")
@patch("backend.routes.admin.QdrantStorageClient.from_config")
...
def test_chat_threads_endpoint(client, mock_redis):
    ...
    mock_redis.hgetall.return_value = {
        b"thread1": b"1700000000.0",
        b"thread2": b"1700000010.0",
    }
...
def test_ingest_memory(mock_check_cap, mock_ensure_schema, client, mock_redis):
    ...
    response = client.post(
        "/api/memory/ingest",
        json={"content": "I like to code in Python", "model": "gpt-4o"},
    )
...
async def test_ensure_schema_exists_missing():
    ...
    with patch("openai.AsyncOpenAI") as mock_openai:
        ...
        mock_redis.set.assert_called_once_with(
            "tenant:test_tenant:schema", '{"entities": [], "relations": {}}'
        )
