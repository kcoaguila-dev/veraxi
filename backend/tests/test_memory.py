...
def run_test(mock_chat):
    ...
    async def mock_invoke(messages):
        ...
        if "favorite color is Quantum Blue" in last_msg:
            return AIMessage(
                content="I will remember that your favorite color is Quantum Blue."
            )
            ...
            if any("Quantum Blue" in m.content for m in messages):
                return AIMessage(
                    content="You told me your favorite color is Quantum Blue."
                )
    ...
    async def run_async_tests():
        ...
        print(f"User: {q1}")
        a1 = await answer_question(
            q1, tenant_id=tenant_id, thread_id=thread_id, return_context=False
        )
        print(f"Agent: {a1}")
        ...
        print(f"User: {q2}")
        a2 = await answer_question(
            q2, tenant_id=tenant_id, thread_id=thread_id, return_context=False
        )
        print(f"Agent: {a2}")
