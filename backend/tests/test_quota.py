...
class TestCheckTenantHardCap:
    def test_under_cap_does_not_raise(self):
        ...
        with patch(
            "backend.storage.quota.Neo4jStorageClient.from_config",
...
class TestMcpInsertHandlerQuota:
    ...
    def test_insert_graph_nodes_enforces_cap_on_cloud(self):
        ...
        with (
            patch(
                "backend.mcp_server.handlers.tool_registry.get_config",
                return_value=_make_config(max_nodes=1000, auth_enabled=True),
            ),
            patch(
                "backend.mcp_server.handlers.tool_registry.check_tenant_hard_cap",
                side_effect=HTTPException(status_code=403, detail="quota"),
            ) as mock_cap,
    ...
    def test_insert_graph_nodes_skips_cap_on_self_hosted(self):
        ...
        with (
            ...
            patch(
                "backend.mcp_server.handlers.tool_registry.insert_graph_nodes",
                return_value={
                    "status": "success",
                    "nodes_inserted": 0,
            ...
            _handle_insert_graph_nodes(
                {"nodes": [], "relations": []}, "local_personal_user"
            )
    ...
    def test_insert_vectors_skips_cap_on_self_hosted(self):
        ...
        with (
            ...
            patch(
                "backend.mcp_server.handlers.tool_registry.insert_vectors",
                return_value={
                    "status": "success",
                    "vectors_inserted": 0,
                    "point_ids": [],
