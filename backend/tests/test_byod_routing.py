...
def test_neo4j_client_uses_byod_context(mock_config):
    ...
    try:
        ...
        with unittest.mock.patch(
            "backend.storage.neo4j_client.GraphDatabase.driver"
        ) as mock_driver:
            Neo4jStorageClient.from_config(mock_config)

            # Assert it used BYOD credentials
            mock_driver.assert_called_once_with(
                "bolt://byod-neo4j:7687", auth=("byod-neo-user", "byod-neo-pass")
            )
    finally:
...
def test_qdrant_client_uses_byod_context(mock_config):
    ...
    try:
        ...
        with unittest.mock.patch(
            "backend.storage.qdrant_client.QdrantClient"
        ) as mock_client:
            QdrantStorageClient.from_config(mock_config)

            # Assert it used BYOD credentials
            mock_client.assert_called_once_with(
                url="http://byod-qdrant:6333", api_key="byod-qdrant-key"
            )
    finally:
...
def test_qdrant_client_falls_back_to_config(mock_config):
    ...
    with unittest.mock.patch(
        ...
        # Assert it used config credentials
        mock_client.assert_called_once_with(
            url="http://default-qdrant:6333", api_key="default-qdrant-key"
        )
